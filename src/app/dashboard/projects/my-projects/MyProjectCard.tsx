...
export default function MyProjectCard({
  ...
  return (
    <li className="relative w-4/5 mx-auto border border-gray-200 p-6 rounded-2xl shadow-lg bg-white transition-transform transform hover:scale-[1.03] hover:shadow-xl">
      <Link href={`/dashboard/projects/my-projects/${project.id}`} className="block space-y-3">
        <h3 className="text-lg font-bold text-[#88b000] tracking-wide">{project.title}</h3>
        ...
        {project.tech_stack && (
          <p className="text-xs text-gray-500">
            <strong className="text-gray-700">Stack tecnológico:</strong> {project.tech_stack.join(", ")}
          </p>
        )}
      </Link>
