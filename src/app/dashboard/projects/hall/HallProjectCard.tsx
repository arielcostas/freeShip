...
export default function HallProjectCard({ project }: { project: any }) {
  ...
  const titleClassName =
    userId && project.author_id === userId ? "my-project-title" : "other-project-title";
    ...
    >
      <Link href={projectUrl} className="block space-y-3">
        ...
        <div className="flex items-center gap-x-3 mb-2">
          ...
          <strong className="text-sm" style={{ color: "var(--text-secondary)" }}>
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="col-span-1 flex flex-col text-xs" style={{ color: "var(--text-secondary)" }}>
            {project.type && (
              <p className="mb-1">
                <strong style={{ color: "var(--text-primary)" }}>Categoría:</strong> {CATEGORY_LABELS[project.type] || "Otra"}
              </p>
            )}
            {project.tech_stack && (
              <p>
                <strong style={{ color: "var(--text-primary)" }}>Stack:</strong> {project.tech_stack.join(", ")}
              </p>
            )}
          </div>

          {/* Columna 2: Descripción del Proyecto */}
          <div className="col-span-3 text-sm leading-relaxed line-clamp-2" style={{ color: "var(--text-primary)" }}>
            {project.description}
          </div>
        </div>
